...
geometry_msgs::Pose go_up_left(int when=0)
{
	...
	pose.position.x=0.557261;
	pose.position.y=0.529211;
	pose.position.z=0.418216;

	pose.orientation.x=-0.0316595;
	pose.orientation.y=0.999455;
	pose.orientation.z=0.00277608;
	pose.orientation.w=0.00896268;
...
geometry_msgs::Pose go_pre_pick(string arm)
{
	...
	if(arm=="left")
	{
		...
		pose.position.x=0.480663;
		pose.position.y=0.782274;
		pose.position.z=0.353923;

		pose.orientation.x=0.367108;
		pose.orientation.y=-0.930026;
		pose.orientation.z=0.00760043;
		pose.orientation.w=0.015044;
...
geometry_msgs::Pose storage_box_position_reconstruct(ros::NodeHandle &nh)
{
	double p_x,p_y,p_z,o_x,o_y,o_z,o_w;

	geometry_msgs::Pose pose;

	nh.getParam("/storage_p_x",p_x);
	nh.getParam("/storage_p_y",p_y);
	nh.getParam("/storage_p_z",p_z);

	nh.getParam("/storage_o_x",o_x);
	nh.getParam("/storage_o_x",o_y);
	nh.getParam("/storage_o_x",o_z);
	nh.getParam("/storage_o_x",o_w);
...
int main(int argc,char**argv)
{
	...
	hole_target=bulb_tester_position_reconstruct(nh);
	storage_box=storage_box_position_reconstruct(nh);
	...
	current_pose_left.orientation.x=0.0374331;
	current_pose_left.orientation.y=0.690262;
	current_pose_left.orientation.z=-0.0411711;
	current_pose_left.orientation.w=0.721416;
